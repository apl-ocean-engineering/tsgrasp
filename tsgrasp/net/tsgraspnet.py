...
from .modules.MinkowskiEngine import initialize_minkowski_unet
from .tsgrasp_super import TSGraspSuper
...
def multi_pointcloud_to_4d_coords(pcs):
    ...
    B, T, N, D = pcs.shape
    batch_coords = torch.arange(
        B, dtype=pcs.dtype, device=pcs.device
    ).repeat_interleave(T * N)
    time_coords = (
        torch.arange(T, dtype=pcs.dtype, device=pcs.device)
        .repeat_interleave(N)
        .repeat(B)
    )
...
def unweighted_sum(coords: torch.Tensor):
    ...
    unique_coords, idcs, counts = coords.unique(
