...
from .tsgraspnet import (
    TSGraspNet,
    discretize,
    multi_pointcloud_to_4d_coords,
    unweighted_sum,
)
...
class LitTSGraspNet(pl.LightningModule):
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            params=[{"params": self.parameters(), "name": "minkowski_graspnet"}],
            lr=self.learning_rate,
        )
        lr_scheduler = {
            "scheduler": torch.optim.lr_scheduler.ExponentialLR(
                optimizer, self.lr_decay
            ),
            "name": "learning_rate",
        }
    ...
    def forward(
        ...
        torch.Tensor, torch.TensorType, torch.Tensor, torch.Tensor, torch.Tensor
    ]:
        ...
        Args:
            positions (torch.Tensor): (batch, t, n_pts, 3) point cloud

        Returns:
            class_logits (torch.Tensor): (batch, t, n_pts, 1) classification logits
            baseline_dir (torch.Tensor): (batch, t, n_pts, 3) gripper baseline direction
            approach_dir (torch.Tensor): (batch, t, n_pts, 3) gripper approach direction
            grasp_offset (torch.Tensor): (batch, t, n_pts, 1) gripper width
            pred_points  (torch.Tensor): (batch, t, n_pts, 1) contact points that were evaluated. The same as positions, in this implementation.
        """
        batch, t, n_pts, _ = positions.shape
        ...
        stensor = SparseTensor(coordinates=coords, features=feats)

        # Make predictions
        class_logits, baseline_dir, approach_dir, grasp_offset = self.model.forward(
            stensor
        )

        class_logits = class_logits.reshape(batch, t, n_pts, 1)
        baseline_dir = baseline_dir.reshape(batch, t, n_pts, 3)
        approach_dir = approach_dir.reshape(batch, t, n_pts, 3)
        grasp_offset = grasp_offset.reshape(batch, t, n_pts, 1)

        return class_logits, baseline_dir, approach_dir, grasp_offset, positions

    def _step(self, batch: dict, batch_idx: int, stage: Optional[str] = None) -> dict:
        ...
        # (batch, t, n_pts, 3) cam point cloud
        grasp_tfs = batch["cam_frame_pos_grasp_tfs"]
        # (batch,) list of (t, N_GT_GRASPS_i, 4, 4) tensors of homogeneous grasp poses (from positive grasps) in the camera frame
        contact_pts = batch["pos_contact_pts_cam"]
        # (batch,) list of (t, N_GT_GRASPS_i, 2, 3) tensors of gripper contact points (for left and right fingers) in the mesh frame

        num_batches, t, n_pts, _ = positions.shape

        # Make predictions
        class_logits, baseline_dir, approach_dir, grasp_offset, _ = self.forward(
            positions
        )
        ...
        width_losses = []
        class_losses = []
        add_s_loss, class_loss, width_loss = 0.0, 0.0, 0.0
        for b in range(num_batches):
            # Compute labels
            pt_labels_b, width_labels_b = self.model.class_width_labels(
                contact_pts[b], positions[b], self.model.pt_radius
            )
            ...
            add_s_loss += add_s_loss_b / num_batches
            width_loss += width_loss_b / num_batches
            class_loss += class_loss_b / num_batches
        ...
        # Log loss components
        self.log(
            f"{stage}_loss", float(loss), on_step=True, on_epoch=True, sync_dist=True
        )
        self.log(
            f"{stage}_add_s_loss",
        ...
        self.log(
            f"{stage}_pt_accuracy",
            float(
                np.mean(
                    [accuracy(pred, label) for pred, label in zip(pt_preds, pt_labels)]
                )
            ),
            on_step=True,
            on_epoch=True,
        ...
        self.log(
            f"{stage}_pt_true_pos",
            float(
                np.mean(
                    [
                        true_positive(pred, label)
                        for pred, label in zip(pt_preds, pt_labels)
                    ]
                )
            ),
            on_step=True,
        ...
        self.log(
            f"{stage}_pt_false_pos",
            float(
                np.mean(
                    [
                        false_positive(pred, label)
                        for pred, label in zip(pt_preds, pt_labels)
                    ]
                )
            ),
        ...
        self.log(
            f"{stage}_pt_true_neg",
            float(
                np.mean(
                    [
                        true_negative(pred, label)
                        for pred, label in zip(pt_preds, pt_labels)
                    ]
                )
            ),
        ...
        self.log(
            ...
            float(
                np.mean(
                    [recall(pred, label) for pred, label in zip(pt_preds, pt_labels)]
                )
            ),
            on_step=True,
            on_epoch=True,
